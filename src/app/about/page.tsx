...
import { apiGateway, Committee } from '@/lib/apiClient';
...
export default function AboutPage() {
  ...
  useEffect(() => {
    async function loadCommittees() {
      try {
        const list = await apiGateway.getCommittees();
        setCommittees(list);
      } catch (err) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background">
      ...
      <main className="flex-grow">
        ...
        {/* COMMITTEES & GROUPS SECTION */}
        <section className="py-20 bg-white">
          <div className="responsive-container">
            <div className="text-center mb-16 space-y-4">
              <Badge className="bg-primary/5 text-primary border-primary/10 rounded-full px-6 py-1 font-bold">
                Our Committees & Leadership Groups / समितियां
              </Badge>
              <h2 className="text-3xl md:text-5xl font-bold text-slate-900">
                PPA Committees & Teams
              </h2>
              <p className="text-slate-500 font-medium max-w-2xl mx-auto">
                Discover our various committees and dedicated groups working tirelessly for pensioner welfare.
              </p>
            </div>
            ...
            {loadingCommittees ? (
              <div className="text-center py-12">
                ...
                <p className="text-sm text-slate-400 mt-2">Loading committees...</p>
              </div>
            ) : committees.length === 0 ? (
              <div className="text-center py-12 text-slate-400 font-medium">
                No committees listed at the moment / कोई समिति वर्तमान में सूचीबद्ध नहीं है।
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
                {committees.map((c) => (
                  <Card key={c.id} className="border border-slate-100 shadow-sm hover:shadow-md transition-all rounded-3xl overflow-hidden bg-white flex flex-col h-full group p-6">
                    <CardContent className="p-0 flex flex-col items-center text-center gap-4 flex-grow">
                      <div className="relative w-44 h-44 rounded-full overflow-hidden border-4 border-primary/10 shadow-md bg-slate-50 shrink-0">
                        {c.image_url ? (
                          <img 
                            src={c.image_url} 
                            alt={c.name} 
                      ...
                      <div className="space-y-1">
                        <h3 className="text-lg font-bold text-slate-900 font-headline leading-snug">{c.name}</h3>
                        <p className="text-sm font-bold text-primary">{c.description || 'Committee Member'}</p>
                      </div>
                    </CardContent>
                  </Card>
                ))}
