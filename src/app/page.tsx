...
export default function Home() {
  ...
  const [stats, setStats] = useState<{ total_members: number; total_events: number } | null>(null);
  const [gallery, setGallery] = useState<any[]>([]);

  useEffect(() => {
    async function loadStats() {
      try {
        const data = await apiGateway.getStats();
        if (data) {
          setStats({
        ...
        console.error('Failed to load stats on homepage:', err);
      }
    }
    async function loadGallery() {
      try {
        const data = await apiGateway.getGallery();
        if (data) {
              ...
              <div className="space-y-8 animate-in fade-in slide-in-from-right duration-700">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="p-6 bg-white rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-all">
                    ...
                    <h4 className="text-3xl font-bold text-slate-900">{stats ? stats.total_members : '500+'}</h4>
                  ...
                  <div className="p-6 bg-white rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-all">
                    ...
                    <h4 className="text-3xl font-bold text-slate-900">{stats ? stats.total_events : '75+'}</h4>
                    <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">Events Conducted</p>
                  </div>
                </div>
        ...
        <section className="py-20 bg-slate-50">
          <div className="responsive-container">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {(gallery.length > 0 ? gallery : galleryItems).map((item, i) => {
                const imageUrl = item.image_url || (Array.isArray(item.image_urls) ? item.image_urls[0] : (typeof item.image_urls === 'string' ? item.image_urls.split(',')[0] : '')) || item.imageUrl || '';
